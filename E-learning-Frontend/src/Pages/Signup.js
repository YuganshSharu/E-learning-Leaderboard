...
const Signup = (props) => {
  ...
  const onSubmitHandler = async (event) => {
    ...
    // const curr_rank = await axios.get(`${BACKEND_URL}//api/student/size`);
    const enteredData = {
      fullName,
      email,
      password,
  ...
  return (
    <Row
      style={ROW_STYLE}
      className="min-vh-100 align-items-center justify-content-center bg"
    >
      <Col xs={11} sm={9} md={6} lg={4}>
        <Form
          onSubmit={onSubmitHandler}
          className="border my-5 border-dark-50 p-4 shadow-lg rounded-3 bg-white"
        >
          <Row className="">
            <Col
              className={`text-center mx-3 pb-2 ${
                role === 'student'
          ...
          <Row className="fs-2 py-2 fw-bold justify-content-center">Signup</Row>
          ...
          <Form.Group className="mb-3" controlId="formBasicDiscordId">
            ...
            <Form.Control
              onChange={onDiscordIdChangeHandler}
              type="text"
          ...
          {error && (
            <Row className="justify-content-center text-danger my-2">
              {error}
            </Row>
          )}
          {isLoading && (
